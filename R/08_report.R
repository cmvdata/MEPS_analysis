# 08_report.R
# MEPS High-Cost Patients - Step 8: Render Analytical Summary
#
# Generates a Markdown narrative for the post-Productos 1-6 refactor and
# attempts to render to HTML via rmarkdown::render. If pandoc is unavailable
# the .Rmd and a stripped .md fallback are still saved.
#
# Inputs read for dynamic table:
#   output/tables/table_twopart_profiles.csv
# Outputs:
#   docs/analytical_summary.Rmd
#   docs/analytical_summary.md      (fallback when pandoc missing)
#   docs/analytical_summary.html    (only if pandoc available)
...
# ---- Build dynamic supplementary table from P6 CSV ------------------------
twopart_table_md <- ""
twopart_csv <- "output/tables/table_twopart_profiles.csv"
if (file.exists(twopart_csv)) {
  tp <- read.csv(twopart_csv)
  rows <- apply(tp, 1, function(r) {
    sprintf(
      "| %s | %s | %.3f (%.3f) | $%s ($%s) | $%s ($%s) |",
      r[["insurance_status"]],
      r[["morbidity_level"]],
      as.numeric(r[["Pr_Y_pos"]]),
      as.numeric(r[["Pr_Y_pos_SE"]]),
      formatC(round(as.numeric(r[["E_Y_given_pos"]])),
              format = "d", big.mark = ","),
      formatC(round(as.numeric(r[["E_Y_given_pos_SE"]])),
              format = "d", big.mark = ","),
      formatC(round(as.numeric(r[["total_E_Y"]])),
              format = "d", big.mark = ","),
      formatC(round(as.numeric(r[["total_E_Y_SE"]])),
              format = "d", big.mark = ",")
    )
  })
  twopart_table_md <- paste0(
    "\n\n### Supplementary table - full two-part profile predictions\n\n",
    "*Reference profile: age 45, Female, Middle Income (200-400% FPL), South, ",
    "Good self-rated health, no functional limitation, no 2022 inpatient stay. ",
...
subtitle: 'How insurance modulates the morbidity-cost gradient in MEPS 2022'
...
- **Cancer active vs. remission proxy.** We use `cancer_ever AND any_inpatient_2022` as a proxy for active cancer. Outpatient chemotherapy patients without 2022 admissions are misclassified as remission. HC-242B condition-level data would refine this, at the cost of substantial pipeline complexity.
...
cat("Saved: docs/analytical_summary.Rmd\n")

# Markdown fallback (without YAML), readable when pandoc is unavailable
md_body <- sub("^---\\s*\\n.*?\\n---\\s*\\n", "", rmd_content, perl = TRUE)
writeLines(md_body, "docs/analytical_summary.md")
cat("Saved: docs/analytical_summary.md\n")

# Try to render to HTML; fail gracefully if pandoc not available
tryCatch({
  rmarkdown::render("docs/analytical_summary.Rmd", output_dir = "docs", quiet = TRUE)
  cat("Rendered: docs/analytical_summary.html\n")
}, error = function(e) {
  cat("HTML rendering failed (pandoc likely missing). Markdown still saved.\n")
  cat("Error message:", e$message, "\n")
  cat("Available: docs/analytical_summary.Rmd and docs/analytical_summary.md\n")
})
